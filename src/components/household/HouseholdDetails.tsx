...
import { MembersLeaderboard } from "@/components/profile/MembersLeaderboard";
...
export const HouseholdDetails = ({
  ...
  const copyInviteCode = () => {
    if (household.invitation_code) {
      navigator.clipboard.writeText(household.invitation_code);
      toast({
        title: "Copied!",
        description: "Invitation code copied to clipboard"
      });
    } else {
      // Fallback to household number if no invitation code
  ...
  return <div className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center space-x-2">
            <p className="text-sm text-gray-500">
              {household.invitation_code 
                ? `Invitation Code: ${household.invitation_code}`
                : `Household Number: ${household.household_number}`}
            </p>
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="icon" onClick={copyInviteCode} className="h-6 w-6">
    ...
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-2">
      <MembersLeaderboard householdId={household.id} />
